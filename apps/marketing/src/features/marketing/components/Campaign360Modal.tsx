import { Target, BarChart3, Users, DollarSign, Calendar, Globe, MapPin, ArrowUpRight, TrendingUp } from 'lucide-react';
...
export default function Campaign360Modal({ campaign, onClose }: Campaign360ModalProps) {
  ...
  const budget = campaign.approvedBudgetCents || 0;
  const roasCalc = rawSpend > 0 ? rawRev / rawSpend : 0;
  const roas = Number.isFinite(roasCalc) ? roasCalc.toFixed(2) : '0.00';
  const pacingPct = budget > 0 ? Math.round((rawSpend / budget) * 100) : 0;
    ...
    <Modal open={true} onClose={onClose} title="Campaign 360 Drilldown" maxWidth="max-w-5xl">
      <div className="flex flex-col md:flex-row gap-6">
        <div className="w-full md:w-80 flex-shrink-0 space-y-6">
          <div className="rounded-2xl border border-stone-200/80 bg-stone-50/50 p-5">
            <h3 className="font-bold text-stone-900 flex items-center gap-2 mb-4 text-sm">
              <Target className="h-4 w-4 text-brand-primary" /> Active Creatives &amp; Placements
            </h3>
        ...
        <div className="w-full md:w-[400px] space-y-6">
          <div className="rounded-2xl border border-stone-200/80 bg-white p-5 shadow-sm">
            <h3 className="font-bold text-stone-900 flex items-center gap-2 mb-4 text-sm">
              <BarChart3 className="h-4 w-4 text-status-success" /> Performance &amp; ROAS
            </h3>
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="rounded-xl bg-status-success/10/50 p-3 border border-emerald-100/50">
                <p className="text-[10px] font-bold uppercase tracking-wider text-status-success mb-1">ROAS</p>
              ...
              <div className="rounded-xl bg-stone-50 p-3 border border-stone-100">
                ...
                <p className="text-xl font-bold text-stone-900">{formatCents(rawSpend)}</p>
              </div>
            </div>

            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-stone-600 flex items-center gap-2"><Users className="h-4 w-4 text-stone-400" /> Attributed Leads</p>
                <p className="font-bold text-stone-900">{Math.floor(rawSpend / 4500)}</p>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-stone-600 flex items-center gap-2"><Calendar className="h-4 w-4 text-stone-400" /> Appts Booked</p>
                <p className="font-bold text-stone-900">{Math.floor(rawSpend / 15000)}</p>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-stone-600 flex items-center gap-2"><DollarSign className="h-4 w-4 text-stone-400" /> Revenue Driven</p>
                <p className="font-bold text-status-success">{formatCents(rawRev)}</p>
              </div>
            </div>
              ...
              <div className="space-y-1 text-xs">
                {[
                  ...
                  <div key={l.name} className="flex items-center justify-between bg-stone-50 p-2 rounded-lg border border-stone-200/70 text-[11px]">
                    ...
                    <div className="text-right font-semibold text-stone-700">{l.value} · <span className="text-status-success">CPL {l.cpl}</span></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mt-6 pt-5 border-t border-stone-100">
              <div className="flex items-center justify-between mb-3">
                ...
                <p className="text-[11px] font-bold text-stone-500">{pacingPct}% of Budget</p>
              </div>
              <div className="h-2 w-full bg-stone-100 rounded-full overflow-hidden">
                <div className="h-full bg-brand-primary rounded-full" style={{ width: `${Math.min(100, pacingPct)}%` }} />
              </div>
            </div>
          </div>
