      ...
      <div className="flex-1 overflow-y-auto px-4 py-5">
        {activeTab === 'agenda' && (
          <div className="space-y-4">
            ...
            <div 
              className="bg-white p-4 rounded-2xl shadow-sm border border-status-warning/20 ring-1 ring-status-warning/20 cursor-pointer active:scale-[0.98] transition-transform"
              onClick={() => setSelectedAppointmentId('appt-2')}
            >
              <div className="flex justify-between items-start">
                <div>
                  <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-status-warning mb-2">Fitting (VIP)</span>
                ...
                <div className="flex flex-col items-end">
                  <div className="w-8 h-8 rounded-full border border-dashed border-rose-300 bg-brand-soft flex items-center justify-center text-brand-primary mb-1">
                    <UserMinus className="h-4 w-4" />
                  </div>
                  <span className="text-[10px] font-bold text-brand-primary">Unassigned</span>
                </div>
              </div>
              <button 
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-stone-100">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-stone-50 rounded-xl p-2.5 text-center border border-stone-100">
                  ...
                  <p className="font-bold text-brand-primary text-lg">100%</p>
                </div>
              </div>
            </div>

            <div className="bg-brand-soft p-4 rounded-2xl shadow-sm border border-border-subtle">
              <div className="flex items-start gap-3 mb-4">
                <div className="w-12 h-12 rounded-full bg-brand-soft flex items-center justify-center text-sm font-bold text-brand-primary-hover shrink-0">
                  MF
                </div>
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-bold text-brand-secondary text-sm">Megan Fox</p>
                      <p className="text-xs text-brand-primary-hover font-medium">Bridal Stylist</p>
                    </div>
                    <span className="inline-flex px-2 py-0.5 rounded text-[10px] font-bold bg-rose-200 text-brand-secondary">Callout (Sick)</span>
                  </div>
                  <div className="mt-2 text-xs text-brand-primary-hover grid grid-cols-1 gap-1">
                    <p><span className="text-brand-primary">Shift:</span> 9:00 AM - 5:00 PM</p>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="bg-white/50 rounded-xl p-2.5 text-center border border-border-subtle">
                  <p className="text-[10px] font-bold text-brand-primary uppercase tracking-wider mb-0.5">Appts</p>
                  <p className="font-bold text-brand-secondary text-lg">2</p>
                </div>
                <div className="bg-white/50 rounded-xl p-2.5 text-center border border-border-subtle">
                  <p className="text-[10px] font-bold text-brand-primary uppercase tracking-wider mb-0.5">Capacity</p>
                  <p className="font-bold text-brand-primary text-lg">0%</p>
                </div>
              </div>
              <button 
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                    <div className="h-full bg-status-success w-3/4 rounded-full"></div>
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-stone-500">Master Fitters</span>
                    <span className="font-bold text-brand-primary">0 / 2</span>
                  </div>
                  <div className="h-2 bg-stone-100 rounded-full overflow-hidden">
                    <div className="h-full bg-brand-primary w-[10%] rounded-full"></div>
                  </div>
                  <p className="text-[10px] text-brand-primary mt-1">Warning: No Master Fitters currently clocked in.</p>
                </div>
              </div>
            </div>
          </div>
