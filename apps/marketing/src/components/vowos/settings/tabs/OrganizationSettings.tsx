...
export function OrgSettingsTab({ onDirtyChange, registerSaveRef, resetTrigger }: Props) {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'profile' | 'brands' | 'locations' | 'sites'>('profile');
  ...
  const handleRefresh = () => {
    ...
    toast({ title: 'Syncing structure', description: 'Pulling latest organization data...' });
    setTimeout(() => {
      load().then(() => {
        setIsRefreshing(false);
        toast({ title: 'Sync Complete', description: 'Organization data is up to date.' });
      });
    }, 1500);
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 py-10 text-sm text-stone-500">
        <Loader2 className="h-4 w-4 animate-spin" /> Loading your organization structure...
      </div>
    );
  }
      ...
      <div className="rounded-2xl border border-stone-200 bg-white shadow-xs">
        <div className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-stone-900">Organization Settings</h3>
              <p className="text-xs text-stone-500">
                Manage your organization structure, brands, locations, and websites.
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="border-t border-stone-200 px-5 flex items-center gap-6">
          {[
            ...
            { id: 'sites', label: 'Websites & Routing', icon: Globe2 }
          ].map(tab => (
            <button
              key={tab.id}
                  ...
                  return (
                    <div key={brand.id ?? `draft-${index}`} className="rounded-2xl border border-stone-200 bg-white p-5 shadow-xs">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        <button type="button" className={secondaryActionCls} onClick={() => { setActiveSubTab('locations'); setTimeout(() => scrollToSection('locations'), 100); }}>Locations</button>
                        <button type="button" className={secondaryActionCls} onClick={() => { setActiveSubTab('sites'); setTimeout(() => scrollToSection('brand-websites'), 100); }}>Websites</button>
                  ...
                  <div id={`brand-profile-${index}`} key={brand.id ?? index} className="grid scroll-mt-24 gap-3 rounded-xl border border-stone-200 p-4 sm:grid-cols-2">
                    ...
                    {brand.id && (
                      <div className="sm:col-span-2 flex flex-wrap gap-2 pt-1">
                        <button type="button" className={secondaryActionCls} onClick={() => { setActiveSubTab('locations'); setTimeout(() => scrollToSection('locations'), 100); }}>Manage locations</button>
                        <button type="button" className={secondaryActionCls} onClick={() => { setActiveSubTab('sites'); setTimeout(() => scrollToSection('brand-websites'), 100); }}>Manage websites</button>
