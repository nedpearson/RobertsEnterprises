-- 20260830000002_growth_intelligence_command_center.sql
-- Location-aware, tenant-safe marketing intelligence foundation.
-- Runs after 20260830000001_growth_social_and_meta.sql and extends the existing
-- canonical growth_ad_campaigns / growth_ad_metrics tables rather than creating
-- a duplicate campaign model.
...
-- ---------------------------------------------------------------------------
-- Extend the existing canonical paid-campaign tables with operational truth.
-- Platform-reported conversions remain separate from VowOS-verified outcomes.
-- ---------------------------------------------------------------------------
ALTER TABLE public.growth_ad_campaigns
  ADD COLUMN IF NOT EXISTS location_id uuid REFERENCES public.locations(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS currency_code text NOT NULL DEFAULT 'USD',
  ADD COLUMN IF NOT EXISTS metadata jsonb NOT NULL DEFAULT '{}'::jsonb;

ALTER TABLE public.growth_ad_metrics
  ADD COLUMN IF NOT EXISTS qualified_leads integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS appointments_booked integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS appointments_attended integer NOT NULL DEFAULT 0,
  ...
  ADD COLUMN IF NOT EXISTS platform_reported_conversions numeric(14,4) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS synced_at timestamptz;

CREATE INDEX IF NOT EXISTS idx_growth_ad_campaigns_location
  ON public.growth_ad_campaigns (business_id, location_id, network, status);
...
-- NULL location ids are intentionally allowed, so use an expression unique
-- index instead of a UNIQUE table constraint to keep business-wide competitors
-- deduplicated as well.
CREATE UNIQUE INDEX IF NOT EXISTS uq_growth_competitors_scope_name
  ON public.growth_competitors (business_id, COALESCE(location_id, '00000000-0000-0000-0000-000000000000'::uuid), lower(name));
