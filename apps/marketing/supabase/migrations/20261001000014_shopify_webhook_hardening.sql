...
-- - database-level canonical Shopify connection uniqueness

-- A permanent Shopify store must never be bound to more than one VowOS
-- connection. Fail closed if historical duplicates exist instead of silently
-- choosing one tenant/brand and risking cross-tenant data routing.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
      FROM public.growth_provider_connections
     WHERE provider = 'shopify'
       AND NULLIF(btrim(metadata->>'shopDomain'), '') IS NOT NULL
     GROUP BY lower(btrim(metadata->>'shopDomain'))
    HAVING count(*) > 1
  ) THEN
    RAISE EXCEPTION
      'Duplicate Shopify shopDomain bindings exist in growth_provider_connections; resolve them before applying Shopify hardening';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION
      'Duplicate Shopify external_account_id bindings exist in growth_provider_connections; resolve them before applying Shopify hardening';
  END IF;
END
$$;

CREATE UNIQUE INDEX IF NOT EXISTS uq_growth_shopify_shop_domain
  ON public.growth_provider_connections (lower(btrim(metadata->>'shopDomain')))
  WHERE provider = 'shopify'
    AND NULLIF(btrim(metadata->>'shopDomain'), '') IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_growth_shopify_external_account
  ON public.growth_provider_connections (btrim(external_account_id))
  WHERE provider = 'shopify'
    AND NULLIF(btrim(external_account_id), '') IS NOT NULL;
...
-- same parent business could collide on the same numeric order id.
