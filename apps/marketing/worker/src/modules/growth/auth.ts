 ...
 * request data until membership is verified. Multi-business users must send
 * X-Business-Id; single-business users remain backward compatible.
 */
...
const requestedBusinessId = (req: Request): string | null => {
...
export async function requireGrowthAccess(req: Request, res: Response, next: NextFunction) {
  ...
  if (selectedBusinessId) membershipQuery = membershipQuery.eq('business_id', selectedBusinessId);
  ...
  if (!selectedBusinessId && memberships.length > 1) {
    return res.status(409).json({
      error: 'This account belongs to multiple businesses. Send X-Business-Id for the active tenant.',
