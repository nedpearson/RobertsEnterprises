...
app.use(async (req, res, next) => {
  ...
  if (authHeader?.startsWith('Bearer ')) {
    ...
    if (!error && user) {
      ...
      // Zero rows means the selected tenant is not authorized. More than one
      // row without X-Business-Id means the caller must select a workspace.
      // Tenant-scoped routes fail closed via their own business-context guards.
    }
  }
