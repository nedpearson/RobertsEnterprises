...
describe('POST /api/auth/login', () => {
  ...
  it('returns JWT for valid demo credentials', async () => {
    const res = await request(app)
      .post('/api/auth/login')
      .send({ email: 'owner@demo.vowos', password: 'demo123' });
    // demo seeder creates this user — if credentials differ just check not 500
    expect([200, 401]).toContain(res.status);
    if (res.status === 200) expect(res.body).toHaveProperty('redirect_url');
  });
});
...
describe('Protected routes — valid token → 200', () => {
  for (const route of PROTECTED_ROUTES) {
    it(`GET ${route} returns 200`, async () => {
      ...
      if (res.status !== 200) {
        console.error('FAILED ROUTE', route, res.status, res.body);
      }
      expect(res.status).toBe(200);
