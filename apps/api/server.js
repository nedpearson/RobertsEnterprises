...
function authenticate(req, res, next) {
  ...
  try {
    ...
    next();
  } catch(e) { console.error('AUTH_ERR:', e.message); return res.status(401).json({ error: 'Unauthorized' }); }
}
...
app.post('/api/demo-login', async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: demoOwner.id, name: demoOwner.first_name, role: demoOwner.role, boutique_id: demoOwner.boutique_id, tenant_id: demoOwner.boutique_id },
      JWT_SECRET,
      { expiresIn: '8h' }
    );
